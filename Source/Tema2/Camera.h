...
namespace Laborator
{
	...
	public:
		...
		void TranslateRight(float distance)
		{
			...
			//Proiectie pe Ox
			glm::vec3 vector_x = glm::dot(glm::vec3(1, 0, 0), right) * glm::vec3(1, 0, 0);
			//Proiectie pe Oz
			glm::vec3 vector_z = glm::dot(glm::vec3(0, 0, 1), right) * glm::vec3(0, 0, 1);

			position += glm::normalize(vector_x + vector_z) * distance;
		}
		...
		void RotateFirstPerson_OZ(float angle)
		{
			...
			// Don't forget to normalize the vectors
			forward = glm::normalize(glm::vec3(glm::rotate(glm::mat4(1), angle, glm::vec3(0, 0, 1)) * glm::vec4(forward, 1)));
